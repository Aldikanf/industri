...
// Tambah siswa
elseif (isset($_POST['add_karyawan'])) {
	...
	if ($nis==="" || $email==="" || $pwd==="" || $nama==="" || $jk==="" || $sklh==="") {
		header("location:../add_karyawan&st=4");
	}else {
		if ($pwd !== "$pwd_cek") {
			header("location:../add_karyawan&st=5");
		} else {
				...
				if ($cek_nis==0) {
					if ($statement= $conn->prepare($sql_user) AND $statement1 = $conn->prepare($sql_detail)) {
						...
						if ($statement->execute() && $statement1->execute()) {
							header("location:../add_karyawan&st=1");
						} else {
							header("location:../add_karyawan&st=2");
						}
					} else {
						header("location:../add_karyawan&st=2");
					}
				} else {
					header("location:../add_karyawan&st=6");
				}
				$conn->close();
			} else {
				header("location:../add_karyawan&st=3");
			}
		}
	}
...
elseif (isset($_POST['edit_karyawan'])) {
	...
	if ($nis==="" || $id==="" || $nama==="" || $jk==="" || $sklh==="") {
		header("location:../karyawan&id=$id&st=4");
	}else {
		if ($statement= $conn->prepare($sql_detail)) {
				...
				if ($statement->execute()) {
					header("location:../karyawan&id_karyawan=$id&st=1");
				} else {
					header("location:../karyawan&id_karyawan=$id&st=2");
				}
			} else {
				header("location:../karyawan&id_karyawan=$id&st=2");
			}
		...
		if ($cek_nis==0) {
			...
			header("location:../karyawan&id=$id&st=6");
		}*/
		$conn->close();
...
// Delete siswa
elseif (isset($_GET['del_karyawan'])) {
	$id = mysqli_real_escape_string($conn, $_GET['del_karyawan']);
	...
	if ($stmt= $conn->prepare($sql_d) AND $stmt1 = $conn->prepare($sql_u)) {
		...
		if ($stmt->execute() && $stmt1->execute()) {
			header("location:../karyawan&st=3");
		} else {
			header("location:../karyawan&st=5");
		}
	} else {
		header("location:../karyawan&st=5");
	}
